...
class RoleController extends Controller
{
    public function index()
    {
        if (Auth::user()->role_id <= 2) {
            $lims_role_all = Roles::where('is_active', true)->where('id', '>', 4)->orWhere('id', '<', 2)->get();
    ...
    public function setPermission(Request $request)
    {
        ...
        if ($request->has('rf-quotes-index')) {
            $permission = Permission::firstOrCreate(['name' => 'rf-quotes-index']);
            if (!$role->hasPermissionTo('rf-quotes-index')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('rf-quotes-index');

        if ($request->has('rf-quotes-add')) {
            $permission = Permission::firstOrCreate(['name' => 'rf-quotes-add']);
            if (!$role->hasPermissionTo('rf-quotes-add')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('rf-quotes-add');

        if ($request->has('rf-quotes-edit')) {
            $permission = Permission::firstOrCreate(['name' => 'rf-quotes-edit']);
            if (!$role->hasPermissionTo('rf-quotes-edit')) {
                $role->givePermissionTo($permission);
            }
        } else
            $role->revokePermissionTo('rf-quotes-edit');

        if ($request->has('rf-quotes-delete')) {
            $permission = Permission::firstOrCreate(['name' => 'rf-quotes-delete']);
            if (!$role->hasPermissionTo('rf-quotes-delete')) {
                $role->givePermissionTo($permission);
            }
        } else
