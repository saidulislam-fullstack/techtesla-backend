...
class HomeController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        if (Auth::user()->role_id == 5) {
            ...
            // dd($customer);
            $lims_sale_data = Sale::with('warehouse')->where('customer_id', ($customer->id ?? null))->orderBy('created_at', 'desc')->get();
            $lims_payment_data = DB::table('payments')
                ...
                ->get();
            $lims_quotation_data = Quotation::with('biller', 'customer', 'supplier', 'user')->orderBy('id', 'desc')->where('customer_id', ($customer->id ?? null))->orderBy('created_at', 'desc')->get();

            $lims_return_data = Returns::with('warehouse', 'customer', 'biller')->where('customer_id', ($customer->id ?? null))->orderBy('created_at', 'desc')->get();
